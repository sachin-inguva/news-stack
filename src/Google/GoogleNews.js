    ...
    useEffect(()=>{
        async function fetchData(){
          //  let keyword='Stevens Institute Of Technology';
            try{
                console.log("hi");
                const { data } = await axios.get(`https://newsapi.org/v2/everything?q=+${keyword}+&apiKey=5aa1805d7d0742469916d9c601a1ba49`);
			...
			<Grid item xs={12} sm={6} md={4} lg={3} xl={2} key={article.content}>
				<Card className={classes.card}>
						...
						<a target='_blank' href={article.url} rel="noreferrer">
								...
								<img className={classes.media} src={article.urlToImage ? article.urlToImage :noImage}
								title='show image'
								alt="No Image"
							/>
